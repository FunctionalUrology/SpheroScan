...
SpheroScan consists of following two main modules:  
  - <ins>**Prediction Module** </ins>: The Prediction Module uses previously trained Deep Learning models to analyze spheroid images and generate a CSV file with the area and intensity of each detected spheroid. 
    - Input <br> <br> To use the prediction module, you must first upload a zipped folder containing your spheroid images. These images can be in .jpeg, .tif, or .jpg format from an Incucyte or microscope platform, but can also be from other platforms as long as there is only one spheroid per image and the background contrast is similar to Incucyte or microscope images.
    ...
    - Output  <br><br> <div style="text-align: right">  Once the images are uploaded, you can click the submit button to begin the masking process. After the masking is complete, a download button will appear, allowing you to download the zipped result file. The result file includes all images with masked spheroids, along with a spheroidArea.csv file. The spheroidArea.csv file generated by the Prediction Module contains the area and intensity information of each masked spheroid, and will serve as an input file for the Visualization Module to create plots and perform statistical analysis. </div> 
    - Prediction Module Screenshot
<p align="center"><img src="./assets/predMod.png" width="80%" align="middle"> </p>   
<br><br>

  - <ins>**Visualization  Module** </ins>: <div style="text-align: right"> The Visualization Module enables the user to interpret the output of the Prediction Module by providing various types of graphical representations and statistical analysis. The plots generated by this module are suitable for publishing and can be exported as high-resolution images in PNG format. </div>
  
      - Input <br> <br> 
        - **Area file:** The spheroidArea.csv file, generated by the Prediction Module, contains information about the area and intensity of each masked spheroid. An example of a area file is available for reference at https://github.com/FunctionalUrology/SpheroScan/blob/main/example-Input-data/area_file.csv.
        - **Metadata File:**  <div style="text-align: right"> A separate .csv file that includes additional information about the images in the Area File. The first column of the Metadata File, named ```filename``` should match the filenames in the Area File. Additionally, the Metadata File should contain at least two more columns that specify the group membership of each image. For example, these columns could indicate which treatment or time poit the spheroid image belongs to. An example of a metadata file is available for reference at https://github.com/FunctionalUrology/SpheroScan/blob/main/example-Input-data/meta_file.csv. </div>
